...
# TODO: add argument support for this
DEB_ARGS=(
...
echo -n "[${KERNEL_PATCH_VER} ${KERNEL_SCHEDULER} ${KERNEL_TYPE}] Would you like to auto-fill NEW configs? [y/N]: ";
read yno;
case $yno in
    [yY] | [yY][Ee][Ss] )
...
case $yno in
    ...
    [yY] | [yY][Ee][Ss] )
        ;&
    *)
        fakeroot debian/rules ${DEB_ARGS[*]} editconfigs;
        ;;
esac
...
case $yno in
    ...
    *)
        echo "*** Starting build... ✓";
        fakeroot debian/rules ${DEB_ARGS[*]} binary-headers "binary-${KERNEL_FLAVOUR}" binary-perarch;
        ;;
esac
